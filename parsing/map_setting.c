...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/08 10:04:03 by mdarify           #+#    #+#             */
/*   Updated: 2023/04/08 10:56:30 by mdarify          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**read_map(t_game *game)
{
	...
	i = 0;
	fd = open(game->fd, O_RDONLY);
	if (fd <= 0)
	{
		write(2, "ERROR : --->FD DOESN'T OPENING\n", 32);
		exit(1);
	}
	game->size = get_file_size(game->fd);
	line = get_next_line(fd);
...
void	fsetting(t_game *game)
{
	...
	if (!game->map->starting_pos)
	{
		write(2, "ERROR : THE PLAYER DOESN'T FOUND\n", 34);
		exit(1);
	}
}
