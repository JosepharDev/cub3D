...
int	is_wallv(t_player *p, int px, int py)
{
	t_point	increment;
...
void	fdda_vertical(t_player *p)
{
	...
	distance.y = TILE_SIZE;
	grid.y = floor(p->y / TILE_SIZE) * TILE_SIZE + increment.y;
	distance.x = fabs(fabs(p->y - grid.y)
			/ tan(translate_deg_to_rad(p->rayangle)));
	grid.x = p->x + (distance.x * increment.signx);
	distance.x = fabs(TILE_SIZE / tan(translate_deg_to_rad(p->rayangle)));
